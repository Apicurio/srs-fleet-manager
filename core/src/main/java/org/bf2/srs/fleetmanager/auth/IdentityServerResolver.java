...
public class IdentityServerResolver implements TenantConfigResolver {
    ...
    @Inject
    Instance<TenantConfigBean> tenantConfigBean;
    ...
    public Uni<OidcTenantConfig> resolve(RoutingContext routingContext, TenantConfigRequestContext requestContext) {
        ...
        if (tenantConfigBean.isResolvable()) {
            return Uni.createFrom().item(tenantConfigBean.get().getDefaultTenant().getOidcTenantConfig());
        } else {
