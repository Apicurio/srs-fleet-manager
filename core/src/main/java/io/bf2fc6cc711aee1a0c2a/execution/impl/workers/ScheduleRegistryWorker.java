...
import static io.bf2fc6cc711aee1a0c2a.execution.impl.workers.WorkerType.SCHEDULE_REGISTRY_W;
import static io.bf2fc6cc711aee1a0c2a.rest.model.RegistryDeploymentStatusValueRest.AVAILABLE;
import static io.bf2fc6cc711aee1a0c2a.rest.model.RegistryDeploymentStatusValueRest.fromValue;
...
public class ScheduleRegistryWorker extends AbstractWorker {
    ...
    public void execute(Task aTask, WorkerContext ctl) throws StorageConflictException {
        ...
        List<RegistryDeployment> eligibleRegistryDeployments = storage.getAllRegistryDeployments().stream()
                .filter(rd -> fromValue(rd.getStatus().getValue()) == AVAILABLE)
                .collect(toList());
